...
namespace structures 
{
		...
		if (this != &other)
		{
			if (size_ == other.size_)
			{
				*vector_ = *other.vector_;
				//*vector_.operator=(*other.vector_);
				//vector->operator=(*other.vector_);
			}
			else
			{
	...
	{
		//TODO 02: Array
		//takto po riadkoch z cvika:
		//int indexVoVectore = mapFunction(index);
		//byte* bPtrVoVectore = vector_->getBytePointer(indexVoVectore);
		//T* TPtrVoVectore = reintepret_cast<T*> (bPtrVoVectore);
		//return *TPtrVoVectore;
	...
	inline int Array<T>::mapFunction(const int index) const
	{
		...
		//range kontroluje uz getbytepointer
	}
}
