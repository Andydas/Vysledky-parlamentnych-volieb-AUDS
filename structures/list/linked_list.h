...
namespace structures
{
	...
	public:
		...
		void setRight(LinkedListItem<T>* next);
	private:
		/// <summary> Nasledujuci prvok zretazeneho zoznamu. </summary>
		LinkedListItem<T>* right_;
	};
	...
	template<typename T>
	inline LinkedListItem<T>* LinkedListItem<T>::getRight()
	{
		return right_;
	}

	template<typename T>
	inline void LinkedListItem<T>::setRight(LinkedListItem<T>* next)
	{
		right_ = next;
	}
	...
	{
		//TODO 04: LinkedList
		*this = other;		//do mna prirad toho druheho
	...
	inline void LinkedList<T>::insert(const T& data, const int index)
	{
		...
		else
		{
			...
			else
			{
				...
				//dovnutra
				LinkedListItem<T>* predok = getItemAtIndex(index - 1);
				LinkedListItem<T>* item = new LinkedListItem<T>(data);
				item->setRight(predok->getRight());
				predok->setRight(item);
				size_++;
			}
		}
	...
	{
		//TODO 04: LinkedList
		LinkedListItem<T>* zmazany;
		
		//odstranenie zmazany zo zretazenia
		if (index == 0)
		{
			//mazem prvy
			if (size_ == 1)
		...
		else
		{
			...
			{
				LinkedListItem<T>* predok = getItemAtIndex(index - 1);
				zmazany = predok->getRight();
				predok->setRight(zmazany->getRight());
			}
			else  //ak mazem z konca
			{
				LinkedListItem<T>* predok = getItemAtIndex(index - 1);
				zmazany = predok->getRight();
				predok->setRight(zmazany->getRight());
	...
	inline int LinkedList<T>::getIndexOf(const T& data)
	{
		//TODO 04: LinkedList
		int index = 0;
		LinkedListItem<T>* temp = first_;
		while (temp != nullptr)
		{
			if (temp->accessData() == data)
			{
				return index;
			}
			temp = temp->getRight();
	...
	{
		//TODO 04: LinkedList
		//zabijem, prisuniem, posuniem
		if (size_ == 0)
		{
			return;
		}
		LinkedListItem<T>* mazany = first_;
		LinkedListItem<T>* dalsi = mazany->getRight();
		while (mazany != nullptr)
		{
			delete mazany;
	...
	{
		//TODO 04: LinkedList
		DSRoutines::rangeCheckExcept(index, size_, "Nesedi index");
