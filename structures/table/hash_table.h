...
namespace structures
{
	...
	{
	private:
		static const int DATA_ARRAY_SIZE = 4000;
	public:
	...
	inline T HashTable<K, T>::remove(const K& key)
	{
		...
		{
			throw std::logic_error("Nenajdeny kluc");
		}
	}
	...
	inline int HashTable<K, T>::hashFunction(const K& key) const
	{
		...
		return (static_cast<long long>(key) * (key + 3)) % DATA_ARRAY_SIZE; //premenil som si na long long lebo to nefungovalo
	}
