...
namespace structures 
{
	...
	inline SequenceTable<K, T>& SequenceTable<K, T>::operator=(const SequenceTable<K, T>& other)
	{
		...
		if (this != &other)
		{
			...
			{
				list_->add(new TableItem<K, T>(*otherItem));
			}
		}
		return *this;
	...
	inline T & SequenceTable<K, T>::operator[](const K key)
	{
		...
		{
			throw std::out_of_range("SequenceTable<K, T>::operator[]: Kluc neexistuje");
		}
	...
	{
		//TODO 09: SequenceTable
		//toto ma bude na semestralke spomalovat
		//if (!containsKey(key))
		//{
		...
		/*else
		{
			throw std::logic_error("SequenceTable<K, T>::insert: Tabulka uz obsahuje item");
		}*/
	...
	{
		//TODO 09: SequenceTable
		//pouzivam foreach aby sa nestalo ze nahodou budem linkedlist prechadzat forkom
		for (TableItem<K,T>* item: *list_)
		{
			if (item->getKey() == key)
			{
